...
build() {
	if [ -f ./output/packer-Ubuntu_14-04.raw.gz ]
		then
...
pack() {
	if [ -f ./output/packer-Ubuntu_14-04.raw ]
		then
			echo -e "${green}started packing...${NC}" && \
			pv -tpreb output/packer-Ubuntu_14-04.raw | pigz > output/packer-Ubuntu_14-04.raw.gz && \
			rm output/packer-Ubuntu_14-04.raw
			echo -e "${green}Image successfully compressed${NC}"
			echo -e "${NC}Filesize: $(ls -lrth output | awk 'NR==1{print$2}')"
	elif [ -f ./output/packer-Ubuntu_14-04.raw.gz ]
		then
...
upload() {
	if [ -f ./output/packer-Ubuntu_14-04.raw.gz ]
		then
			...
			lftp -c "open -u root sftp://localhost:2222; put -O /var/tmp/image output/packer-Ubuntu_14-04.raw.gz" && \
...
deploy() {
	...
	gunzip -f packer-Ubuntu_14-04.raw.gz && \
	DATE=$(date +"%H%M_%d_%m_%Y") && \
	DATE1=$(echo $DATE) && \
	mv /var/tmp/image/packer-Ubuntu_14-04.raw "/var/tmp/image/packer-Ubuntu_14-04_$DATE1.raw" && \
	source ~/openrc && \
	glance image-create --name "Ubuntu 14.04 $DATE1" --container-format bare --disk-format qcow2 --is-public true --file packer-Ubuntu_14-04_$DATE1.raw'
}
